...
test('displays projects page', async ({ page }) => {
  ...
  await expect(page).toHaveTitle(
    'Projets · Clément Latzarus · développeur web'
  );
  await expect(page.getByRole('heading', { name: 'Biblys' })).toBeVisible();
});
...
test('displays blog post page', async ({ page }) => {
  await page.goto(
    'blog/how-to-run-matomo-in-a-docker-container-while-passing-database-credentials-as-environment-variables/'
  );
  ...
  await expect(
    page.getByRole('heading', {
      name: 'How to run Matomo in a Docker container while passing database credentials as environment variables',
    })
  ).toBeVisible();
});
